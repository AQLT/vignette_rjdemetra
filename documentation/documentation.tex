...
\item
  ...
  \code{regarima_spec_def_x13()} and \code{regarima_def_tramoseats()}
  for the RegARIMA model and \code{x13_spec_def()} and
  \code{tramoseats_spec_def()} for the entire seasonal adjustment model
  (including the pre\_adjustment with a RegARIMA model).
\item
...
Once the specification it's created, the regARIMA model can be performed
...
For more details see the help of the correponding function or the
...
an outlier or a user-defined regresso you have specify the
transformation function (\code{transform.function}, it cannot be
automatic). Those checks are done each time a new specification it's
create. Therefore, some specifications cannot be setted in two staged.
For example, fixing the coefficient of an outlier has to be done the
save time the outliers are defined. The following code doesn't fix the
