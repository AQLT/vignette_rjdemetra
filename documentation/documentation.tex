...
\Plaintitle{\pkg{RJDemetra}: An R Interface To JDemetra+ Seasonal Adjustment
Software}
\Shorttitle{\pkg{RJDemetra}: An R Interface To JDemetra+ Seasonal Adjustment
Software}
...
The package \pkg{RJDemetra} provides an R interface to the seasonal
...
\begin{longtable}{lll}
\caption{\label{tab:obj_tab_x13}SA object structure (seasonal adjustment made with \code{x13})}\\
\toprule
...
\begin{longtable}{lll}
\caption{\label{tab:obj_tab_tramoseats}SA object structure (seasonal adjustment made with \code{tramoseats})}\\
\toprule
Object & Level & Type [RJDemetra S3 class]\\
\midrule
sa\_object & 0 & list [SA, X13]\\
...
used in \pkg{RJDemetra}. For more details see the help of the
