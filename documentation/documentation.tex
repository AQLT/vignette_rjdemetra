...
empty unless additional output was requested by the user (see next
sub-sections). Finally, when estimating RegARIMA only the regarima
object is created.
...
(\code{arma}), model quality measures (\code{loglik}), regarima
specification after it's estimation with the estimated effects
...
\code{which}. Table x summarizes all the graphs available for the
\code{sa_object}, as well as its \code{plot()} options.
...
(\code{mode}; e.g.: additive). Then, the \code{decomposition_X11} object
comprises quality measures on the decomposition (\code{mstats}), namely
the \(M\) and \(Q\) statistics. It contains also the final unmodified
si-ratios \(d8\) and final seasonal factors \(d10\) (\code{si_ratio}),
as well as the information on the final seasonal filter
(\code{s_filter}) and trend filter(\code{t_filter}). The graph below
ilustrates the si-ratios.

\begin{CodeChunk}

\begin{CodeInput}
R> plot(sax13$decomposition)
\end{CodeInput}


\begin{center}\includegraphics{img/img-unnamed-chunk-8-1} \end{center}
