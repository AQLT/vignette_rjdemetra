...
The method of decomposing the pre-adjusted series differs between TRAMO-SEATS+ and X-12ARIMA/X-13ARIMA. In TRAMO-SEATS+, SEATS ("Signal Extraction in ARIMA Time Series") decomposes the observed series with a ARIMA-model based method. Whereas in X-12ARIMA/X-13ARIMA, the X11 algorithm decomposes the time series by means of linear filters. More information on the TRAMO-SEATS+ method can be found on the Bank of Spain website ([link](www.bde.es)) and on X-12ARIMA/X-13ARIMA, on the U.S. Census Bureau website.
...
More details on the methodlogy used in JDemetra+ can be found in the JDemetra+ manuals and user guides available at [link](https://ec.europa.eu/eurostat/cros/content/documentation_en).
...
mp1 <- get_object(wk, 1) # to get the first multiprocessing
count(mp1) # there are two seasonal adjustment models in the multiprocessing
sa_item1 <- get_object(mp1, 1) # to get the first seasonal adjustment model
get_name(sa_item1) # the name of the seasonal adjustment model in JDemetra+
raw_ts <- get_ts(sa_item1) # to get the input raw time series

compute(wk)
sa_model1 <- get_model(sa_item1, workspace = wk)
```

\code{get_ts()} and \code{get_model()} can also be used directly to the workspace or a multiprocessing to import all the raw times series or all the seasonal adjustment model:
...
```{r, eval=FALSE}
# To get all the raw time series of the workspace:
all_raw_ts <- get_ts(wk)
# To get all the seasonal adjustment models of the multi-processing "mp1"
